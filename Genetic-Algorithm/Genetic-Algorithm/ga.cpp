...
int SCENARIO[S] = {8500, 45000, 3800,  8500, 600, 45000, 800,
                   1600, 2300,  45000, 800,  500

};

int MAXIMUM[K] = {1772, 250,
                  9500, //원래는 15000 //2020 07 26 9000에서 9375로 바꿈, 10000,
                        //11000으로 바꿈.
                  792,  1772, 125,
                  9500, //원래는 15000
                  167,  105,  355, 250, 522,
...
double WEIGHT[K] = { // dnn 으로 만든 회귀식의 파라미터
    0.00296492,  -0.01752832, -0.06862973, -0.00023781, 0.05772761, 0.05088029,
    0.10909722,  -0.02881844, 0.05099791,  0.01761491,  0.0308822,  0.05687409,
    -0.02485382, -0.07651293, 0.06452318,  -0.05187816};
...
Chromosome::Chromosome() {
    ...
    for (int row = 0; row < M; row++) {
        for (int col = 0; col < K; col++) {
            ...
            // print_chromo();
            // data[row][col] = (rand() % ((int)(2 * (double)R[row] / K) - LB +
            // 1)) + LB; data[row][col] = C[col] + rand() % (int)(C[col] *
            // RAND_PERCENTAGE);
            ...
            if (rand_binary == 0)
            {
                    if (C[col] == 0)
                    {
                            data[row][col] = MIN_ASN + rand() % (int)(MIN_ASN *
            RAND_PERCENTAGE);
                    }
                    else
                    {
                            data[row][col] = C[col] + rand() % (int)(C[col] *
            RAND_PERCENTAGE);
                    }
            }
            else
...
Post: Calculates the cumulative sum of rows and columns and stores them in the
array row_sum and column_sum
*/
...
and randomly changes the data if the generated real number is less than the
...
void Chromosome::repair() {
    ...
    for (int col = 0; col < K; col++) // 지역마다
    {
        ...
        {
            int excess_value = 0;
            excess_value = (data[0][col] - MAXIMUM[col]) /
                           (K - 1); //넘는값을 체크, 넘는값/15

            for (int i = 0; i < col;
                 i++) //해당컬럼 전의 지역들에 15빵한걸 넣어줌
            {
                if ((data[0][i] + excess_value) <
                    MAXIMUM[i]) //이걸 더해도 상한을 안넘으면 넣어줌
                {
    ...
    while (1)
    {
            ...
            for (int col = 0; col < K; col++) //flag check, 원래는 MAX_ASN이
    C[col]
            {
                    if (column_sum[col] > MAXIMUM[col]) flag_arr[col] =
    FLAG_EXCESS; else if (column_sum[col] < MAXIMUM[col]) flag_arr[col] =
    FLAG_SHORTAGE; else flag_arr[col] = FLAG_SAME;
            }
            ...
            if (count > 999900) //디버깅용 임시코드
                    cout << "excess_col is " << data[rand_row][excess_col] << ",
    " << rand_col << "th column" << endl;
            ...
            if (count > 999900) //디버깅용 임시코드
                    cout << "shortage_col is " << data[rand_row][shortage_col]
    << ", " << rand_col << "th column" << endl;
            ...
            if (data[rand_row][shortage_col] < data[rand_row][excess_col])
            {
                    ...
                    if ((data[rand_row][shortage_col] < KEEP + LB) &&
    (data[rand_row][excess_col] >= KEEP + LB))
                    {
                            //cout << "old shortage col is " <<
    data[rand_row][shortage_col] << " and excess col is " <<
    data[rand_row][excess_col] << endl;

                            int temp = 0;
                            temp = data[rand_row][shortage_col] + KEEP;
                            data[rand_row][shortage_col] =
    data[rand_row][excess_col] - KEEP; data[rand_row][excess_col] = temp;
                            //cout << "swapping with keep: shortage col is "<<
    data[rand_row][shortage_col] << " and excess col is " <<
    data[rand_row][excess_col] << endl;
                    }
                    else
                    {
                            ...
                            data[rand_row][shortage_col] =
    data[rand_row][excess_col]; data[rand_row][excess_col] = temp;
                    }

                    //If the if statement does not swap properly

                    if ((old_excess_data == data[rand_row][excess_col]) &&
    (old_shortage_data == data[rand_row][shortage_col]))
                    {
                            int temp = 0;
                            temp = data[rand_row][shortage_col];
...
*/
void Chromosome::evaluate() // It's a temporary code, and we'll have to fix it
                            // later
{
    // cout << "evaluate" << endl;
    double eval_sum = 0;
    ...
    */
    ...
    int weight = 1;
    double time[S];
    //서로게이트 모델: 시뮬레이션 함수, 가중치 부분 주석처리하고 딥러닝으로 얻은
    //웨이트값 넣어서 평가하긔 그리고 나서 베스트 배치안은 시뮬레이션에 넣어
    //작업시간이랑 적합도 평가
...
Post: Individuals are created for a defined population size. Compute the fitness
statistically and put the value into the variable
*/
...
void Population::share() {
    // cout << "share" << endl;
    double euclidean_distance[POP_SIZE][POP_SIZE] = {
        0,
    };
    double sigma = 0; // maximum E-distance
    double share_func[POP_SIZE][POP_SIZE] = {
        0,
    };
    double sigma_s[POP_SIZE] = {
        0,
    }; // sum of sharing function
...
double Population::calc_euclidean_distance(int point1, int point2) {
    ...
    for (int row = 0; row < M; row++) {
        for (int col = 0; col < K; col++) {
            pow_sum += pow(individual[point1].get_data(row, col) -
                               individual[point2].get_data(row, col),
                           2);
        }
    }
    return sqrt(pow_sum);
...
int Population::select() {
    ...
    for (int i = 0; i < POP_SIZE; i++) {
        ...
        // cout << "in for"<< idx << "," << sum_of_fitness << "," << sum <<
        // endl;
    }

    point = sum_of_fitness * (double)rand() / RAND_MAX;
    // cout << "after make point" << idx << "," << sum_of_fitness << "," << sum
    // << endl; cout << "point : " << point << endl;
...
void Population::crossover(int parent1, int parent2) {
    ...
    for (int row = 0; row < M; row++) {
            for (int col = 0; col < K; col++)
            {
                    if (row < m && col < k)
                    {
                            offspring.put_data(row, col,
    individual[parent1].get_data(row, col));

                    }
                    else if (row < m && col >= k)
                    {
                            offspring.put_data(row, col,
    individual[parent2].get_data(row, col));
                    }
                    else if (row >= m && col < k)
                    {
                            offspring.put_data(row, col,
...
void Population::replacement(int parent1, int parent2) {
    ...
    distance1 = (double)fabs(individual[parent1].get_fitness() -
                             offspring.get_fitness());
    distance2 = (double)fabs(individual[parent2].get_fitness() -
                             offspring.get_fitness());
    /*
    //  < Maximizing code >
    if (distance1 <= distance2) {
    if( (parent1 != best_index) && (individual[parent1].get_fitness() <
    offspring.get_fitness()))	//elitism individual[parent1] = offspring;
    }
    else {
    if ((parent2 != best_index) && (individual[parent2].get_fitness() <
    offspring.get_fitness()))	//elitism individual[parent2] = offspring;
    }
    */
...
void Population::print_best_assignment() {
    ...
    for (int row = 0; row < M; row++) {
        for (int col = 0; col < K; col++) {
            // cout.width(K);
            cout << setw(3) << individual[best_index].get_data(row, col)
                 << endl;
            if (Asn_data.is_open()) {
...
void assignment_eval(int assignment[K]) {
    ...
    cout << "assignment's fitness is: " << basic_fitness << " "
         << time_weight_fitness << endl;
